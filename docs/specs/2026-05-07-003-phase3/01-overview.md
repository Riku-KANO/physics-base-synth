...
- **Thiran allpass を全 voice で採用 (D36 案 D)**: A1〜C6 で Lagrange 比 ~4000× のピッチ精度向上 (A4 で 0.0002%)。C8 は damping<1 の物理限界で ignore 継続（Lagrange/Thiran 共通の問題）
- Brightness LPF 群遅延補正がディレイ長 1 行補償（`adjusted_length = raw_len − τ_g(brightness)`）で `note_on` 時に適用される（D37、Thiran 採用後も brightness LPF 由来の偏移は残るため依然必要）
...
| **D36** | Thiran allpass の採否 | **案 D 採用（全 Thiran + C8 ignore 継続）**。Step 1 試作で A1〜C6 が Thiran で劇的改善（A4: Lagrange 0.89% → Thiran 0.0002%、~4000×）、C8 は Lagrange/Thiran 共通で `damping=0.9999` の loop gain<1 が原因の物理限界（仕様書原案の案 A 条件「C8 自己発振成立」は damping<1 では原理的に達成不可）のため ignore 継続。`KarplusStrong` の補間 field は `thiran: ThiranCoeffs` 単一型に解消、enum dispatch を除去。`Engine::new` は Thiran で構築。Lagrange は Phase 4 再評価のため `LagrangeCoeffs` として残置。 | 03 章 |
| **D37** | Brightness 群遅延補正 | **D36 案 D (Thiran 採用) でも依然必要**: brightness LPF (1 段 IIR) の群遅延 `τ_g = (1 − brightness) / brightness` は補間段とは独立に残るため、`note_on` 時に `adjusted_length = raw_length − τ_g` で補償する。実装: `KarplusStrong::note_on_internal` 内で `brightness.target()` を取得して τ_g を計算、`raw_len.max(3.0).floor()` から減じる。process 中の brightness 変化はピッチ偏移として許容（vibrato 効果）。Phase 4 で allpass 直列補正を再評価 | 03 章 |
