...
| 指標 | Phase 3 後実測 | Phase 4a target | 撤退ライン |
|---|---|---|---|
| WASM gzip サイズ | 27.78 KB | < 30 KB（target ~13 KB、wasm-opt -O3 込み） | > 30 KB で R32（Modal 係数削減） |
...
**達成基準**:
- Phase 4a 後 gzip < 15 KB（想定 13 KB、Phase 3 27.78 KB から大幅削減）
- 超過時は `wasm-opt --print-stats` で各 pass の効果を分析、または `--symbols-only` で名前圧縮確認
...
| Step 13 (PresetSelector UI) | F42 実機 |
| Step 14 (LfoSection + ModWheel UI) | F40 / F41 実機 |
| Step 15 (統合 cargo test + alloc ゼロ) | F45 / F46 / F47 |
| Step 16 (実機確認 + F38b 再計測) | F38b 再確認、Phase 4a 全機能の実機確認 |
| Step 17 (ドキュメント整備) | Phase 4a 完成 |

すべての F38b + F39〜F47 が達成された時点で Phase 4a 完成。F46（release timing）と F45（alloc ゼロ）は **Step 15 で必須化**、F38b は **Step 1 + Step 16 の 2 回計測**（Phase 3 検証 + Phase 4a 後の regression 確認）。
