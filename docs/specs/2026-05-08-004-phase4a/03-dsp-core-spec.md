...
├── voice.rs                (Phase 4a で set_lfo_pitch_factor / set_lfo_brightness_offset 委譲を追加)
└── voice_pool.rs           (Phase 4a で set_lfo_pitch_factor / set_lfo_brightness_offset 追加)
```
...
impl Lfo {
    ...
    pub fn process_sample(&mut self) -> f32 {
        // 1. 現在 phase での値を先に計算（初回 phase=0 → sine=0、triangle=-1）
...
fn process(&mut self, output_l: &mut [f32], output_r: &mut [f32]) {
    ...
    for i in 0..n {
        ...
        // D48 Pitch destination: pitch factor は全 voice 共通なので Engine 側で
        // exp2 を 1 回だけ計算し、`pool.set_lfo_pitch_factor(factor)` で fan-out。
        // KarplusStrong 側は乗算のみで処理（per voice exp2 を回避、性能向上）。
        let pitch_offset_semitones = lfo_value * self.lfo_pitch_depth.next_sample()
            * mod_wheel_v * LFO_PITCH_SCALE_SEMITONES;
...
impl<const N: usize> VoicePool<N> {
    /// Phase 4a D48: LFO Pitch factor (Engine 側で exp2 済み) を全 voice に fan-out。
    /// per sample 呼出されるため、fan-out 内では SmoothedValue ではなく
    /// 単純 f32 として保持し、Voice の per-sample 計算で読まれる設計。
    /// `factor = exp2(-pitch_offset_semitones / 12.0)`、Engine 側で 1 回計算済。
...
`Voice` trait と `voice.rs` の委譲も同形式で追加（`set_lfo_pitch_factor` / `set_lfo_brightness_offset`）。
...
pub struct KarplusStrong {
    ...
    // Phase 4a D48 新規
    /// LFO からの pitch factor (Engine 側で `exp2(-semitones / 12)` 計算済、毎 sample 更新)。
    /// `process_sample` で `length_target.next_sample() * lfo_pitch_factor` で動的 length を計算。
    /// 初期値は 1.0（pitch offset 0 と等価、Phase 3 互換）。
...
pub fn process_sample(&mut self) -> f32 {
    ...
    // Phase 4a D48: LFO Pitch factor を実効 length に乗算 (factor は Engine 側で exp2 済み)。
...
### `excitation_snapshot` を `#[cfg(test)]` でガード + 該当 integration test を unit test に移動（D45）
...
**注意**: integration test (`crates/dsp-core/tests/karplus_strong_pick_tests.rs`) からは Rust の `cfg(test)` がライブラリビルド側でオフになるため `excitation_snapshot` が見えなくなる。よって本 Step では **該当 integration test を `karplus_strong.rs` 内の `#[cfg(test)] mod` ブロックに移動**して unit test 化する（Phase 3 既存 7 箇所の `excitation_snapshot` 呼出を含む test 関数）。

**移動先のスケッチ**:
```rust
// karplus_strong.rs 末尾
#[cfg(test)]
mod excitation_tests {
    use super::*;
    // Phase 3 既存の karplus_strong_pick_tests.rs から移動した test 関数群:
    // - test_pick_min_beta_minimal_shape
    // - test_pick_position_node_at_beta_half
    // - test_pick_position_attenuates_kth_harmonic (2 ループ分)
    // - test_pick_internal_k_zero_branch
    // 等、`excitation_snapshot` を参照する test 関数を移動。
    // unit test 文脈では private fields に直接アクセスできるため、
    // `excitation_snapshot` の `Vec::to_vec()` alloc を回避して `&self.buffer[..]`
    // 直接参照に書き換える最適化も可能（任意）。
}
```

これで production builder からは完全に除外。`#[doc(hidden)]` は `#[cfg(test)]` の場合自動的に non-public のため不要。`pnpm build` (release) では `cargo build --target wasm32-unknown-unknown --release` で `#[cfg(test)]` がオフになり、コードは完全削除される。

**Phase 3 既存テスト件数の保持**: integration test → unit test の移動でテスト数は不変、`cargo test -p dsp-core` の合計件数が Phase 3 と同じ (94 PASS + 1 IGNORED) になることを確認する。
...
Engine::process (per sample, n iterations):
  ...
  │                   mod_wheel_v × 0.5                  │
  │ 4. pitch_factor = exp2(-pitch_offset / 12)          │  ← Engine 側で 1 回 exp2
  │    self.pool.set_lfo_pitch_factor(pitch_factor)     │  ← per sample fan-out
  ...
  │     │     │     ├─ effective_length = base × lfo_pitch_factor │ ← Phase 4a 拡張 (factor は Engine 側で exp2 済)
...
Engine 側で exp2 を 1 回だけ計算する設計（D48 修正版、per voice exp2 を回避）:
...
| Engine 内 LFO 適用（offsets 計算） | next_sample × 3 (9) + 乗算 (6) + Pitch exp2 (7) ≈ 22 |
| VoicePool fan-out (set_lfo_pitch_factor × 8 voice + set_lfo_brightness_offset × 8 voice) | 16 |
| KarplusStrong 内の LFO Pitch 適用（base_target × factor、乗算のみ） | 1 × 8 = 8 |
| KarplusStrong 内の LFO Brightness 適用（加算 + clamp） | 2 × 8 = 16 |
| **合計 (sine, 8 voice)** | **11 + 22 + 16 + 8 + 16 = 73 演算/sample** |

WASM 1 GHz 仮定で +73 / 1e9 × 128 = +0.0093 ms/process。Phase 3 想定 1.95 ms に対し +0.5%、修正前 (90 演算 / +0.012 ms) より改善。triangle 波形なら -2 演算で +0.0091 ms。

**修正の意義**: 修正前の設計（per voice で `exp2(-offset/12)` を計算）は exp2 が 7 演算 × 8 voice = 56 演算/sample と支配的。Engine 側で 1 回計算した factor を fan-out する設計で 56 → 7 + 8 = 15 演算（-41 演算 / sample、48% 削減）に改善。
...
| `voice_pool.rs` | 8 音 voice 管理 | `set_lfo_pitch_factor` / `set_lfo_brightness_offset` 追加 |
| `karplus_strong.rs` | KS 単音 | `lfo_pitch_factor` (Engine 側 exp2 済) / `lfo_brightness_offset` フィールド追加、`process_sample` で length に factor 乗算 / brightness に offset 加算、`excitation_snapshot` を `#[cfg(test)]` でガード + 該当 integration test を unit test に移動 |
| `voice.rs` | Voice trait 委譲 | `set_lfo_pitch_factor` / `set_lfo_brightness_offset` 委譲 |
| `traits.rs` | Voice trait 定義 | `set_lfo_pitch_factor` / `set_lfo_brightness_offset` 追加 |
