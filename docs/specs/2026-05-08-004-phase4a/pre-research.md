...
### 8.2 `KarplusStrong::excitation_snapshot` の `#[cfg(test)]` ガード + integration test 移動

**現状**: `Vec::to_vec()` で test only API として alloc しているが、`#[doc(hidden)]` のみで production binary に含まれる可能性。

**対処**:
1. `karplus_strong.rs` で `#[cfg(test)] pub(crate) fn excitation_snapshot(&self) -> Vec<f32>` に変更。production builder からは完全に除外
2. **現状 `crates/dsp-core/tests/karplus_strong_pick_tests.rs` から `excitation_snapshot` が呼ばれている 7 箇所の test 関数を `karplus_strong.rs` 内の `#[cfg(test)] mod` ブロックに移動**（Rust では integration test ビルド時にライブラリ側の `cfg(test)` がオフになるため、`#[cfg(test)]` 関数を integration test から呼ぶことはできない）

**サイズ影響**: ~50 byte の関数 + Vec を除く。微々たるが production cleanliness 向上。

**テスト件数**: integration test → unit test の移動でテスト数は不変、`cargo test -p dsp-core` の合計件数が Phase 3 と同じ (94 PASS + 1 IGNORED) になることを確認。
...
> - **◎ §8.2 excitation_snapshot cfg(test) 化 + integration test 移動**: Phase 4a Step 3 で実装、~5 行 + 7 test 関数の移動（~150 行）
...
13. **Step 13**: PresetSelector + ModWheel + LfoSection UI 実装（§3 / §4 / §5、UI 3 コンポーネント一括）
14. **Step 14**: 統合 cargo test + alloc ゼロ検証 + サイズ計測 + release timing test（F39〜）
15. **Step 15**: 実機確認（pnpm dev で全 Phase 4a 機能動作確認）+ F38b 再計測
16. **Step 16**: ドキュメント整備（README / CLAUDE.md / retrospective 準備）
17. **Step 17**: PR 作成 + main マージ
